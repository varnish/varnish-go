...
`Start()` / `AssertStart(t)` spawns `varnishd` in a temporary workdir under
`/tmp/`, waits for it to be ready, and begins collecting VSL records in the
background. `Stop()` must always be called (use `defer`).
