...
- **New**: `varnish` package — generic, non-test-specific `varnishd` process management, extracted from `vtest`. `varnish.New()` builds a `VarnishBuilder` (listeners via `HTTPListener`/`HTTPSListener`/`UDSSocket`, `Parameter`, `Jail`, `SetEnv`/`ClearEnv`, `SetLicensePath`); `Build()` starts a `Varnish` with `WaitRunning`, `ListenAddr`, `AdmConn`, `LogReaderBuilder`, `StatReaderBuilder`, and `Stop`. `vtest.VarnishTestBuilder` now wraps it, layering test-only conveniences (inline VCL, backend injection, always-on VSL/syslog accumulation) on top
- **Fix**: `varnish`/`vtest` — `Varnish.ListenAddr(name)` returns the resolved, IPv6-safe `"host:port"` for a named listener; `WaitRunning` previously mishandled IPv6 addresses when populating `URL`/`TLSURL`
- **Changed**: `varnish.Varnish.Stop` sends SIGTERM to the varnishd manager instead of issuing a CLI stop; the manager now exits on its own instead of `Stop` waiting out the full `StopTimeout` before force-killing it
...
- **Fix**: `vtest` — `VarnishBuilder.Backend` with an https URL now emits a TLS backend on Varnish Enterprise (certificate verification off, suited to self-signed test backends); previously the scheme was parsed but ignored and the backend spoke plaintext to port 443. On Varnish Cache, `Start` returns an error for https backends
