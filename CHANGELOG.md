...
- **New**: `varnish` package — generic, non-test-specific `varnishd` process management, extracted from `vtest`. `vtest.VarnishTestBuilder` now wraps it, layering test-only conveniences (inline VCL, backend injection, always-on VSL/syslog accumulation) on top
